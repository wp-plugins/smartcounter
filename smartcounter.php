...
function smartcounter_check($the_content) {
	if(strpos($the_content, "[smartcounter:")!==FALSE) {
 		preg_match_all('/(?<name>\w+):(?<digit>\d+)/', $the_content, $matches, PREG_SET_ORDER); 
		foreach($matches as $match) { 
	$the_content = preg_replace("/\[(?<name>\w+):(?<digit>\d+)\]/", smartcounter_tag($match[digit]), $the_content,1);
		}	
	}
    return $the_content;
}
